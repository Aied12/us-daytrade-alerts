# مساعد تنبيهات التداول اليومي — أسهم أمريكية
# تنبيهات فقط — لا ينفّذ أوامر شراء/بيع

## ماذا يفعل؟
- تقرير صباحي: مزاج السوق + أفضل الفرص + ماذا تفعل
- تنبيهات أثناء الجلسة: دخول / راقب / تجنّب مع وقف وهدف وحجم الصفقة
- ملخص مسائي
- إدارة مخاطرة مربوطة برأس مالك (افتراضيًا 45,000 ر.س)
- دفتر صفقات يدوي لمراجعة أدائك

## رأس المال والمخاطرة (الافتراضي)
| البند | القيمة |
|------|--------|
| رأس المال | 45,000 ر.س ≈ $12,000 |
| مخاطرة الصفقة | 1% ≈ 450 ر.س |
| حد الخسارة اليومي | 2% ≈ 900 ر.س |
| أقصى تنبيهات دخول/يوم | 5 |

إذا وصلت حد الخسارة اليومي: **قف عن التداول**.

## التثبيت
```bash
cd us-daytrade-alerts
python3 -m venv .venv
source .venv/bin/activate
pip install -r requirements.txt
cp .env.example .env
```

## الميزات الجديدة
- أزرار: دخلت / راقبت / تجاهلت
- أوامر: `/scan` `/risk` `/journal` `/mode` `/chart` `/voice` `/help`
- وضع مبتدئ أو محترف (`USER_MODE`)
- فلتر أسهم فوق `$5` (`MIN_PRICE_USD`)
- تنبيه عاجل للفرص القوية + إيموجي القوة
- ملخص صوتي قصير + ملصق شارت يومي
- قناة عامة اختيارية عبر `TELEGRAM_CHANNEL_ID` (أضف البوت أدمن في القناة)

## ربط تيليجرام
1. افتح `@BotFather` → `/newbot` → انسخ التوكن
2. افتح بوتك في تيليجرام وأرسل `/start`
3. شغّل:
```bash
source .venv/bin/activate
python scripts/link_telegram.py --token 'ضع_التوكن_هنا'
```
يرسل رسالة تجريبية ويحفظ التوكن و`chat_id` في `.env` تلقائيًا.

## جدولة يومية تلقائية
مثبّتة عبر cron (أيام التداول فقط، و`scheduled_run.py` يتأكد من توقيت نيويورك):

| التقرير | توقيت السعودية |
|--------|----------------|
| تحديث كل 5 دقائق | **11:00 ص → 11:00 م** |
| إذا فيه تغيّر | رسالة «تحديث» + تفاصيل التغيّر |
| إذا ما فيه تغيّر | «تحديث — لا يوجد شي جديد يابطل» |
| ملخص مسائي | ≈ 11:05 م |

```bash
bash scripts/install_cron.sh   # تثبيت / تحديث الجدولة
crontab -l                     # عرضها
ls logs/cron_*.log             # السجلات
```

## تشغيل يدوي
```bash
python main.py risk
python main.py scan --mode demo
python main.py journal
```

## تنبيه قانوني
هذا أداة تعليمية للمساعدة في التنظيم والتنبيهات. ليست نصيحة استثمارية، ولا تضمن ربحًا، ولا تنفّذ أوامر نيابةً عنك.
